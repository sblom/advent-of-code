...
var frontier = new OrderedBag<(IImmutableList<char> keys, char currentKey, int steps)>(
	Comparer<(IImmutableList<char> keys, char currentKey, int steps)>.Create((x,y) => x.steps - y.steps)
) { (ImmutableList<char>.Empty as IImmutableList<char>, '@', 0) };

var maxkeys = 0;

var states = new HashSet<(string, char)>();
...
while (frontier.Count > 0)
{
	...
	var hashkey = (string.Join("", front.keys.OrderBy(ch => ch)), front.currentKey);
	...
	foreach (var key in reachableKeys)
	{
		...
		frontier.Add((next, key.Key, front.steps + key.Value.dist));
	}
}