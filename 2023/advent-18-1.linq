...
var bigdigs = digs.Select(x => (dir: x.color[^1] switch {'0' => 'R', '1' => 'D', '2' => 'L', '3' => 'U'}, int.Parse(x.color[1..^1],NumberStyles.HexNumber))).Dump();
...
foreach (var dig in bigdigs)
{
    ...
    (x, y) = (x + dig.Item2 * dx, y + dig.Item2 * dy);
    path.Add((x,y));
}
...
{
    A2 += (long)path[i].x * path[(i + 1) % path.Count].y - (long)path[i].y * path[(i + 1) % path.Count].x;
}
