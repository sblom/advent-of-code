...
foreach (var line in lines.Reverse())
{
	switch (line)
	{
		...
		case string L when rotate.IsMatch(L):
			...
			if (dir == "left") input = input.Skip(input.Count() - x3).Take(x3).Concat(input.Take(input.Count() - x3)).ToArray();
		...
		case string L when move.IsMatch(L):
			...
			char ch1 = list[int.Parse(match5.Groups["Y"].Value)];
			list.RemoveAt(int.Parse(match5.Groups["Y"].Value));
			list.Insert(int.Parse(match5.Groups["X"].Value), ch1);
			input = list.ToArray();
			break;
		case string L when rotatebased.IsMatch(L):
			...
			{
				if ((i + i + 1 + (i >= 4 ? 1 : 0)) % input.Count() == loc2)
					break;
			}
			if (i == input.Count())
				"asdf".Dump();
			input = input.Skip(i).Concat(input.Take(i)).ToArray();
			input = input.Skip(1).Concat(input.Take(1)).ToArray();
			if (i >= 4) input = input.Skip(1).Concat(input.Take(1)).ToArray();
			break;
		default:
			break;
	}
