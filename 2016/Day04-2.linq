...
Regex rx = new Regex(@"(?<letters>[-a-z]*)-(?<sector>\d+)\[(?<checksum>\w{5})\]");
...
foreach (var line in lines)
{
	...
	foreach (var letter in match.Groups["letters"].Value)
	{
		if (letter == '-') continue;
	...
	var checksum = string.Join("", dict.OrderByDescending(d => d.Value).ThenBy(d => d.Key).Take(5).Select(d => d.Key));
	if (checksum == match.Groups["checksum"].Value)
	{
		int.TryParse(match.Groups["sector"].Value, out var sector);
		match.Groups["sector"].Value.Dump();
		string.Join("", match.Groups["letters"].Value.Select(ch => ch == '-' ? ' ' : (char)(((ch - 'a' + sector) % 26) + 'a'))).Dump();
	}
}
